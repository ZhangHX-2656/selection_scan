...
while(<R>)
     { chomp $_;
       @tm=split(/\t/, $_);
       #if($tm[0] !~ /^Chromosome/){last;}
       #$tm[0] =~ s/Chromosome/chr/;
       @tn=split(/;/, $tm[8]);
       $tn[0] =~ s/ID=//;
       $gene[$i]->[0] = $tn[0];
       $gene[$i]->[1] = $tm[0];
...
for $i(0..1){ printf W "\t%s-%s-dxy(1kb)", $pop_name[$i*2], $pop_name[$i*2+1];
              printf W "\t%s-%s-fst", $pop_name[$i*2], $pop_name[$i*2+1];
            }
...
foreach (@gene)
        { foreach(@pop_name){@{$pop_fr{$_}}=();}
          ...
          while(<R>)
           ...
           my @fst=(); 
           my ($p1,$p2);
           for $i(0..1) { $p1 = $pop_name[$i*2];
                          $p2 = $pop_name[$i*2+1];
           	          $dxy[$i]=cal_dxy(\@{$pop_fr{$p1}},\@{$pop_fr{$p2}},$len);
           	          $dxy[$i]=1000*$dxy[$i];
           	          if($dxy[$i]==0){ $fst[$i]=0;}
           	          else           { $fst[$i]=(2*$dxy[$i]-$pi{$p1}-$pi{$p2})/(2*$dxy[$i]); 
           	                           if($fst[$i]<0) {$fst[$i]=0;}
           	                         }
           	        }
           ...
           for $i(0..1){ printf W "\t%10.5f", $dxy[$i];
                         printf W "\t%10.5f", $fst[$i];
            }
           print W "\n";
         }                            
